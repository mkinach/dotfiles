...
def plot_data(files):
    ...
    # add legend
    ax.legend(bbox_to_anchor=(0.25, 0.25), loc='upper left') if args.legend else ax.legend()
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Plot two-column numeric data from files')
    parser.add_argument('file', metavar='file', type=str, nargs='+', help='input data file(s)')
    parser.add_argument('-x', '--xlabel', action='store', help='x-axis label (must enclose in quotes if including spaces)')
    parser.add_argument('-y', '--ylabel', action='store', help='y-axis label (must enclose in quotes if including spaces)')
    parser.add_argument('-t', '--title',  action='store', help='plot title (must enclose in quotes if including spaces)')
    parser.add_argument('-s', '--scale',  action='store', help='set scaling factor (integer)')
    parser.add_argument('-l', '--legend', action='store_true', help='move legend')
    parser.add_argument('--xmin', action='store', help='set minimum x-value (float)')
    parser.add_argument('--xmax', action='store', help='set maximum x-value (float)')
    parser.add_argument('--ymin', action='store', help='set minimum y-value (float)')
    parser.add_argument('--ymax', action='store', help='set maximum y-value (float)')
