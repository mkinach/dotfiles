...
which xwininfo > /dev/null 2>&1 || { echo 'xwininfo not found. Exiting.' ; exit 1; }
which ffmpeg > /dev/null 2>&1 || { echo 'ffmpeg not found. Exiting.' ; exit 1; }
which slop > /dev/null 2>&1 || { echo 'ffmpeg not found. Exiting.' ; exit 1; }
...
if [[ -z $output_file ]]; then
  ...
  echo " No output filename provided... Saving to default file: ${output_file}"; echo
fi
...
if [[ -n "$wflag" ]]; then
  ...
  echo " Press 'q' to stop recording after selecting the target window."; echo
  ...
  echo " Press 'q' to stop recording after selecting the target area."; echo
